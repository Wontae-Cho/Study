...
public class Ex04DataStructure {
	...
	public static void main(String[] args) {
		...
		int[][] data2 = new int[students.length][];// 인트배열을 갖는 배열. = 다중배열
		...
		System.out.println("--------------------------------------");
		System.out.println("학생성적관리프로그램(ver 0.1.2)");
		System.out.println("--------------------------------------");
		int input = 0;
		while (true) {
			...
			if (input == 1) {
				...
				// 기존 data2보다 하나 더 큰 배열을 가진 배열을 만드는 과정.
				int[][] data3 = new int[data2.length + 1][];
				for (int i = 0; i < data2.length; i++) {// 기존배열 복사
					data3[i] = data2[i];
				}
				// ////////////////////////////////////////////////////////
				int[] stu = new int[4];
				int num = data2[data2.length - 1][0] + 1; // 마지막학번 +1
				stu[0] = num;
				...
				if(i==data2.length){
					...
					//i를 기준으로 뒷부분을 복사. i는 제외되야 함으로 i+1에서부터.
