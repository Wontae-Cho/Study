...
public class JMultiClient implements ActionListener {
	...
	public JMultiClient(String argIp, String argId) {
		ip = argIp;
		id = argId;
	...
	public void init() throws IOException{
		// 소켓을 생성 소켓은 ip하고 포트를 가지고
		socket = new Socket(ip,5000);
		System.out.println("connected...");
		oos= new ObjectOutputStream(socket.getOutputStream());
		ois = new ObjectInputStream(socket.getInputStream());
		JMultiClientThread ct = new JMultiClientThread(this);
	...
	public static void main(String[] args) {
		...
		JMultiClient cc = new JMultiClient(ip,id);
		try {
			cc.init();
			sc.close();
		} catch (IOException e) {
